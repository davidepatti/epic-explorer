...
__PD_m5e_inst_load(const __PD_OP* op)
{
  ...
  /*  Modified by Davide Patti (dpatti@diit.unict.it) on 1 march 2008,
   *  Just skip not assigned cids (e.g. ICMOVE) */
  ...
  if (cid >= 0 && cid < NUM_CLUSTERS)
  {
      ...
      /* See if we got a hit. */
      while (!hit) {
	stall_one_cycle(op, NULL,++stall_cycles, __PD_STALL_INSTRUCTION_LOAD);
	hit = M5E_is_present_in_icache(req, cid);
      }
